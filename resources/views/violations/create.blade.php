<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Data Pelanggaran Siswa') }}
        </h2>
    </x-slot>
    ...
    <div class="py-6 max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            <form action="{{ route('violations.store') }}" method="POST">
                ...
                {{-- Jenis Pelanggaran (Bisa Dicari & Terintegrasi Otomatis) --}}
                <div class="mb-4">
                ...
                {{-- Input tersembunyi/nyata untuk dikirim ke Controller --}}
                <div class="mb-4">
                    <label for="violation_name" class="block font-medium text-sm text-gray-700">Nama Pelanggaran (Teks Tersimpan)</label>
                    <input type="text" name="violation_name" id="violation_name" value="{{ old('violation_name') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-50" readonly required>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-50" readonly required>
                    @error('category')
                        <span class="text-red-600 text-xs">{{ $message }}</span>
                    @enderror
                </div>

                {{-- Jumlah Poin (Otomatis) --}}
                <div class="mb-4">
                    <label for="points" class="block font-medium text-sm text-gray-700">Jumlah Poin</label>
                    <input type="number" name="points" id="points" value="{{ old('points') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-50" readonly required>
                    @error('points')
                        <span class="text-red-600 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', date('Y-m-d')) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                    @error('date')
                        <span class="text-red-600 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" placeholder="Tambahkan catatan jika diperlukan...">{{ old('description') }}</textarea>
                    @error('description')
                        <span class="text-red-600 text-xs">{{ $message }}</span>
                    @enderror
                </div>
    ...
    {{-- CDN jQuery dan Script JS Select2 serta Logika Otomatis --}}
    ...
    <script>
        $(document).ready(function() {
            ...
            // Otomatisasi isi form ketika jenis pelanggaran dipilih
